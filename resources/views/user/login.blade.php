        ...
        <form class="login-form" id="login-form">
            ...
            <div id="step-guest-otp" class="form-step" style="display:none;">
                ...
                <div class="form-group mb-3">
                    ...
                    <input type="text" id="guest-otp" class="form-control" placeholder="Enter OTP" required maxlength="6"
                        inputmode="numeric">
                </div>
                <button type="button" id="btn-verify-guest-otp" class="login-submit-btn w-100 m-0">Verify &amp; Login</button>
                <div class="d-flex justify-content-between align-items-center mt-3">
...
<script>
    ...
    document.getElementById('btn-guest-send-otp').addEventListener('click', () => {
        ...
        if (!val) {
            Swal.fire({ icon: 'warning', title: 'Required', text: 'Please enter your mobile number.', confirmButtonColor: '#1F5552' });
            return;
        }

        if (isMobile(val)) {
            fetch("{{ url('/user/send-guest-otp') }}", {
                method: 'POST',
                headers: { 'Content-Type': 'application/json', 'X-CSRF-TOKEN': csrf },
                body: JSON.stringify({ mobile: val })
            })
    ...
    document.getElementById('btn-verify-guest-otp').addEventListener('click', () => {
        ...
        fetch("{{ url('/user/verify-guest-otp') }}", {
            method: 'POST',
            headers: { 'Content-Type': 'application/json', 'X-CSRF-TOKEN': csrf },
            body: JSON.stringify({ 
                otp: otpVal,
